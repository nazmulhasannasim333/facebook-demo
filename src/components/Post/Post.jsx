...
const Post = ({post}) => {
    const {details, author, image_url } = post;
        ...
        <Card.Body>
          ...
          />
           <div className="ps-2">
            ...
            <p className="fw-normal mt-0" style={{fontSize: '13px'}}>{moment(author?.published_date).format('MMMM D, h:mm A')}</p>
           </div>
          </Card.Title>
          <Card.Text>
            {seeDetail ? <>{details.slice(0, 260)}... <span onClick={() => setSeeDetail(false)} className="text-dark fw-semibold ">See more</span></> : details }
          </Card.Text>
          ...
          {
          user && like ? <span onClick={() => setLike(false)}><FaThumbsUp className="fs-4 text-primary me-2"/></span> : <span onClick={()=> setLike(true)}><FaRegThumbsUp className="fs-4 me-2"/></span> 
          }
          <span>Like</span>
          </div>
          <div className="d-flex align-items-center">
